...
static mss_i2c_instance_t * this_i2c = &g_mss_i2c1;
...
static const uint8_t iic_hdmi_out_config[IIC_HDMI_OUT_CONFIG_LEN][2] =
{
	...
	{0x18, 0xC6}, // Color space conversion
	...
	//
	//SYNC GENRATION
	//HSYNC Placement after DE falling edge - 110 pixels and HYSYNC pulse width - 40 pix// VSYNC placement - 5 HSYNC after the falling edge 1st HSYNC pulse after last EAV of the Active video.
	// VSYNC pulse width - 5 HYSNC pulses
	{0x30, 0x1B},
	{0x31, 0x82},
	{0x32, 0x80},
	{0x33, 0x14},
...
static mss_i2c_status_t hdmi_i2c_tx_write(uint8_t i2c_div_address, uint8_t data_reg, uint8_t data)
{
    ...
    MSS_I2C_write(this_i2c, i2c_div_address, (const uint8_t *)tx_buffer, write_length, MSS_I2C_RELEASE_BUS );
...
{
	mss_i2c_status_t status;
	MSS_I2C_write_read( this_i2c, i2c_div_address, &data_reg, 1, readdata, readlen, MSS_I2C_RELEASE_BUS);
    status = MSS_I2C_wait_complete( this_i2c, MSS_I2C_NO_TIMEOUT );
	return status;
}
...
void HDMI_tx_init()
{
	...
	MSS_I2C_init(this_i2c, 0x01, MSS_I2C_PCLK_DIV_256);

#if 0
    if(hdmi_i2c_tx_write(I2C_MUX_ADDR,0,0x08) == MSS_I2C_SUCCESS)
    {
      printf("transaction successful\n");
    }

    if(hdmi_i2c_tx_write(IO_EXP_ADDR,0x03,0xEA) == MSS_I2C_SUCCESS)
    {
      printf("transaction successful\n");
    }

    if(hdmi_i2c_tx_read(IO_EXP_ADDR, 0x01, &write_data, 1) == MSS_I2C_SUCCESS)
    {
    	printf("transaction successful\n");
    }


    if(hdmi_i2c_tx_write(IO_EXP_ADDR, 0x01, write_data | 0x05) == MSS_I2C_SUCCESS)
    {
      printf("transaction successful\n");
    }
    ...
    if(hdmi_i2c_tx_write(IO_EXP_ADDR, 0x01, write_data & (~0x10)) == MSS_I2C_SUCCESS)
    {
      printf("transaction successful\n");
    }
#endif
    ...
    if(hdmi_i2c_tx_write(I2C_MUX_ADDR,0,0x04) == MSS_I2C_SUCCESS)
    {
      printf("transaction successful\n");
    }
