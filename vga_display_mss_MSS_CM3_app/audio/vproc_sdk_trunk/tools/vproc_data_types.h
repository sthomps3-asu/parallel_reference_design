...
typedef int8 vProcDeviceIdType;
/* external types */
...
typedef struct {
    uint16 buf[128];      /*the firmware data block to send to the device*/
    uint8  numWords;    /*the number of words within the block of data stored in buf[]*/
...
typedef struct {
   ...
   uint16 value[128]; /*the value to write into reg */
} dataArr;
