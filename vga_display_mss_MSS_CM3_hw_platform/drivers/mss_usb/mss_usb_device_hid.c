...
#if 1
#define MAX_PACKET_SIZE 1024 // Inserted by Solutions Team
uint8_t g_rx_data[MAX_PACKET_SIZE] = { 0 }; // Inserted by Solutions Team
uint8_t g_tx_data[MAX_PACKET_SIZE] = { 0 };
extern void USB_PCKHandler();
#endif

/***************************************************************************//**
 * Private Functions
 */
static uint8_t* usbd_hid_get_descriptor_cb(uint8_t recepient, uint8_t type,
		uint32_t* length, mss_usb_device_speed_t musb_speed);

static uint8_t usbd_hid_init_cb(uint8_t cfgidx,
...
static uint8_t usbd_hid_process_request_cb(mss_usbd_setup_pkt_t * setup_pkt,
		uint8_t** buf_pp, uint32_t* length);

static uint8_t usbd_hid_tx_complete_cb(mss_usb_ep_num_t num, uint8_t status);
static uint8_t usbd_hid_rx_cb(mss_usb_ep_num_t num, uint8_t status,
		uint32_t rx_count);
...
mss_usbd_class_cb_t usbd_hid_cb = { usbd_hid_init_cb, usbd_hid_release_cb,
		usbd_hid_get_descriptor_cb, usbd_hid_process_request_cb,
		usbd_hid_tx_complete_cb, usbd_hid_rx_cb, usbd_hid_cep_tx_done_cb,
...
uint8_t hid_conf_descr[HID_CONFIG_DESCR_LENGTH] = {
		...
		//0x00u,                                               /* bInterval */   // Changed by Sudeep
		0x01u, /* bInterval */// Changed by Sudeep
		...
		0x03u, /* bmAttributes */
		//0x0Au,                                              /* wMaxPacketSize LSB */ // Changed by Sudeep
		//0x00u,                                              /* wMaxPacketSize MSB */ // Changed by Sudeep

		0x00u, /* wMaxPacketSize LSB */// Changed by Sudeep
		0x04u, /* wMaxPacketSize MSB */// Changed by Sudeep
		//0x08u                                               /* bInterval */          // Changed by Sudeep
		0x04u /* bInterval */// Changed by Sudeep
...
#if 1
		/* Modified by Solutions Team */
		0x06u, 0x00u, 0xFFu, 0x09u, 0x01u, 0xA1u, 0x01u, 0x15u, 0x00u, 0x26u,
		0xFFu, 0x00u, 0x75u, 0x08u, 0x95u, 0x64u, 0x09u, 0x01u, 0xB1u, 0x00u,
		0xC0u
#else
 ...
 */
void MSS_USBD_HID_init(mss_usb_device_speed_t speed) {
 ...
 * See mss_usb_device_hid.h for details of how to use this function.
 */
uint32_t MSS_USBD_HID_tx_report(uint8_t * buf, uint32_t length) {
 ...
 * See mss_usb_device_hid.h for details of how to use this function.
 */
uint8_t MSS_USBD_HID_tx_done(void) {
...
static uint8_t*
usbd_hid_get_descriptor_cb(uint8_t recepient, uint8_t type, uint32_t* length,
		mss_usb_device_speed_t musb_speed) {
	/*User Selected FS:
	 Operate only in FS
	 User Selected HS:
	...
	if (USB_STD_REQ_RECIPIENT_DEVICE == recepient) {
		if (USB_CONFIGURATION_DESCRIPTOR_TYPE == type) {
			hid_conf_descr[HID_CONF_DESCR_DESCTYPE_IDX]
...
static uint8_t usbd_hid_init_cb(uint8_t cfgidx,
	...
	MSS_USBD_tx_ep_configure(HID_INTR_TX_EP, HID_INTR_TX_EP_FIFO_ADDR,
			HID_INTR_TX_EP_MAX_PKT_SIZE, HID_INTR_TX_EP_MAX_PKT_SIZE, 1u,
			DMA_ENABLE, MSS_USB_DMA_CHANNEL1, MSS_USB_XFR_INTERRUPT,
			NO_ZLP_TO_XFR);
...
static uint8_t usbd_hid_rx_cb(mss_usb_ep_num_t num, uint8_t status,
	...
	if (g_rx_complete_status == 1) {
		//MSS_USBD_HID_rx_report(10); // Modified by Sudeep
		MSS_USBD_HID_rx_report(MAX_PACKET_SIZE); // Modified by Sudeep
		USB_PCKHandler();
 ...
 * See mss_usb_device_hid.h for details of how to use this function.
 */
uint32_t MSS_USBD_HID_rx_report(uint32_t length) {
	if (g_hid_state == USBD_HID_CONFIGURED) {
		MSS_USBD_rx_ep_read_prepare(MSS_USB_RX_EP_1, (uint8_t*) &g_rx_data,
				length);
		return (1u);
	} else {
		return (0u);
